    ...
    <div class="title-list">
      ...
      >
        <template v-slot:content>
          该标签下有 {{ category.count }} 篇文章
        </template>
        <div class="tag-margin" :class="{active: category.id===isActive }" @click="tagClick(category.url, category.id)">
          {{ category.cateName }}
        </div>
      </el-tooltip>
    </div>
...
export default {
  ...
  async asyncData ({ $axios, params, query }) {
    const { data } = await $axios.$get('/articles/category')
    ...
    if (params.url) {
      ...
      url = categoryList[0].url
      isActive = categoryList[0].id
    }
    if (query.page) {
      const { data } = await $axios.$get(`articles/list?categoryUrl=${url}&page=${query.page}`)
      return {
        list: data.list,
        total: data.total,
        isActive,
      ...
      const { data } = await $axios.$get(`articles/list?categoryUrl=${url}&page=1`)
      return {
        list: data.list,
        total: data.total,
        isActive,
  ...
  methods: {
    currentChange (page) {
      this.$router.push({ name: 'category-url', params: { url: this.url }, query: { page } })
    },
    tagClick (url, id) {
      this.$router.push({ name: 'category-url', params: { url, id } })
    },
    formatToDate (v) {
      return formDate(v)
    }
